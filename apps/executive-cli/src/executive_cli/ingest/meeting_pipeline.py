"""Meeting Ingest Pipeline — Pipeline Engine implementation (ADR-12 / D2).

Processes meeting note files as a registered Pipeline Engine pipeline.
...
Backward compatibility: `ingest_meeting_file()` in pipeline.py still works as before.
This module provides `ingest_meeting_via_pipeline()` as the Pipeline Engine alternative.
"""
...
def _step_parse(ctx: StepContext) -> StepResult:
    """Read meeting file, create IngestDocument, and prepare raw text for extraction."""
    session = ctx.session
    ...
    existing = session.exec(
        select(IngestDocument).where(
            IngestDocument.channel == CHANNEL_MEETING,
...
def _step_extract(ctx: StepContext) -> StepResult:
    """Extract task candidates from meeting text using LLM."""
    ...
    try:
        candidates = extract_candidates(
            raw_text=raw_text,
            source_channel=CHANNEL_MEETING,
            context=context,
            provider=provider,
            model=model,
...
def register_meeting_ingest_pipeline() -> None:
    """Register the meeting_ingest pipeline definition."""
...
def ingest_meeting_via_pipeline(
    session: Session,
    *,
    path: str,
    title: str | None,
    ...
    """Process a meeting notes file using the Pipeline Engine.

    Drop-in replacement for `ingest_meeting_file()` that routes through
